...
export class ImagePreprocessor {
  ...
  private fallbackOptions: FallbackProcessingOptions;

  constructor(
    config: PreprocessingConfig, 
    resizeFunc?: ResizeFunction,
    fallbackOptions: FallbackProcessingOptions = {
  ...
  preprocessFrame(frame: Frame): PreprocessingResult {
    ...
    try {
      console.log('Starting preprocessing for frame:', frame.width, 'x', frame.height, 'format:', frame.pixelFormat);
      
      // Check if we have the resize plugin available
      if (this.resizeFunc) {
        return this.processWithResizePlugin(frame, startTime);
      } else {
        return this.processWithFallback(frame, startTime);
      }
  ...
  /**
   * Process frame using the vision-camera-resize-plugin
   */
  private processWithResizePlugin(frame: Frame, startTime: number): PreprocessingResult {
    'worklet';
   ...
   */
  private processWithFallback(frame: Frame, startTime: number): PreprocessingResult {
    'worklet';
    ...
    try {
      ...
      if (frame.pixelFormat === 'yuv') {
        ...
        rgbData = this.convertYUVtoRGB(frameData, frame.width, frame.height);
      ...
      if (targetWidth !== frame.width || targetHeight !== frame.height) {
        ...
        resizedData = this.resizeRGBData(rgbData, frame.width, frame.height, targetWidth, targetHeight);
      ...
      if (this.config.outputFormat === 'uint8') {
        ...
        processedData = this.normalizeUint8ToFloat32(resizedData);
   ...
   */
  private convertYUVtoRGB(yuvData: Uint8Array, width: number, height: number): Uint8Array {
    'worklet';
    ...
    try {
      ...
      throw new Error(`YUV conversion error: ${error.message}`);
    }
  }
 ...
 * @param resizeFunc - Optional resize function from useResizePlugin() hook
 * @param fallbackOptions - Options for fallback processing when resize plugin unavailable
 */
export function createMoveNetPreprocessor(
  resizeFunc?: ResizeFunction,
  fallbackOptions?: FallbackProcessingOptions
): ImagePreprocessor {
  return new ImagePreprocessor(MOVENET_PREPROCESSING_CONFIG, resizeFunc, fallbackOptions);
}

/**
 * Create a preprocessor instance that only uses fallback processing
 * Useful when vision-camera-resize-plugin is not available
 */
export function createFallbackPreprocessor(
  config: PreprocessingConfig = MOVENET_PREPROCESSING_CONFIG,
  fallbackOptions?: FallbackProcessingOptions
): ImagePreprocessor {
  return new ImagePreprocessor(config, undefined, fallbackOptions);
}